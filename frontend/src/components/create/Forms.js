...
function Forms() {
  ...
  const handleFileChange = (event) => {
    ...
    document.getElementById(
      "ArchivoAdjunto"
    ).innerText = "Archivo Subido";
    document.getElementById("ArchivoAdjunto").style.fontWeight = "bold";
    ...
    .then(response => {
        ...
        setNombreArchivo(name_of_file);
        // Realizar acciones adicionales después de enviar el archivo
    })
    .catch(error => {
        console.error('Error al enviar el archivo:', error);
        alert("error subiendo localmente el archivo")
    });
};
  ...
  const handleCreateClick = () => {
    ...
    const data = {
        ...
        "nombre_patente": inputValue,
        "nombre_autor": userData.nombre + " " + userData.apellido,
        "facultad": userData.facultad,
        "descripcion": textareaValue,
        "private_key": userData.token_ethereum
    }
    ...
    .then(result => {
        ...
        navigate('/');
        // Realizar acciones adicionales según la respuesta recibida
    })
    .catch(error => {
    ...
    <>
      <div className="px-2 py-12 ">
        <div className="flex flex-no-wrap items-start">
          <div className="w-8/12 mx-auto max-w-7xl">
                ...
                <div className="mt-10 px-7">
                  ...
                  <div className="grid w-full grid-cols-1 lg:grid-cols-2 md:grid-cols-1 gap-7 mt-7 ">
                    <div>
                      ...
                      <input className="w-full p-3 mt-4 border border-gray-300 rounded outline-none focus:bg-gray-50" type="text" value={inputValue} onChange={handleInputChange}/>
                      <p className="mt-3 text-xs leading-3 text-gray-600">
                        Escoge un título menor a 100 caractéres
                      </p>
                    </div>
                  ...
                  <div className="mt-4 border border-gray-300 rounded">
                    <div className="flex flex-wrap items-center px-4 py-3 border-b border-gray-300">
                      ...
                      <div className="flex items-center justify-end ml-2 gap-x-3 lg:ml-auto md:ml-auto">
                        ...
                        />
                        <p id= "ArchivoAdjunto" className="text-sm leading-none text-gray-600 cursor-pointer" onClick={handleFileClick}>
                          Añadir Archivo
                        </p>
                        <svg
                          className=" "
                    ...
                    <textarea
                      ...
                      placeholder="Empieza escribiendo una descipción aquí ..."
                      ref={textareaRef} 
                    />
                  </div>
                </div>
                <p className="mt-3 text-xs leading-[15px] text-gray-600 px-7">
                  Ingresa una descripción corta de menos de 100 caractéres.
                </p>
                <hr className="h-[1px] bg-gray-100 my-14" />
                <div className="flex flex-col flex-wrap items-center justify-center w-full px-7 lg:flex-row lg:justify-end md:justify-end gap-x-4 gap-y-4">
                  <button className="bg-white border-indigo-700 rounded hover:bg-gray-50 transform duration-300 ease-in-out text-sm font-medium px-6 py-4 text-indigo-700 border lg:max-w-[95px]  w-full " onClick={handleCancelClick}>
                    Cancel
                  </button>
                  <button className="bg-indigo-700 rounded hover:bg-indigo-600 transform duration-300 ease-in-out text-sm font-medium px-6 py-4 text-white lg:max-w-[144px] w-full " onClick={handleCreateClick}>
                    Crear NFT
                  </button>
                </div>
              </div>
